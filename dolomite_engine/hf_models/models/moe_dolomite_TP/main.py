...
class MoEDolomiteForCausalLM_TP(MoEDolomitePreTrainedModel_TP, MoEDolomiteForCausalLM):
    def __init__(self, config: MoEDolomiteConfig, **kwargs) -> None:
        MoEDolomitePreTrainedModel_TP.__init__(self, config, **kwargs)

        self.vocab_size = config.vocab_size

        self.transformer = MoEDolomiteModel_TP(config, **kwargs)
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        config = MoEDolomiteConfig.from_pretrained(pretrained_model_name_or_path)

        # use dummy tensors to avoid initializing model here
        with torch.device("meta"):
            # try sharding vocab matrices if really struggling for memory
            model = MoEDolomiteForCausalLM_TP._from_config(
